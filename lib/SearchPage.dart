...
class CreatorSearchWidget extends StatefulWidget {
  const CreatorSearchWidget({super.key});

  @override
  State<CreatorSearchWidget> createState() => _SearchPageState();
}

class _SearchPageState extends State<CreatorSearchWidget> {
  ...
  List<String> searchResultsCreators = [];
  List<String> searchHistory = [];

  final FirebaseStorage _storage = FirebaseStorage.instance;
  final FirebaseFirestore _firestore = FirebaseFirestore.instance;

  void navigateToProfilePage(String creatorName) {
    Navigator.push(
      context,
      MaterialPageRoute(
          builder: (context) => ProfilePage(creatorName: creatorName)),
    );
  }
  ...
  Future<void> searchCreators(String searchTerm) async {
    setState(() {
    ...
    try {
      ...
      if (querySnapshot.docs.isEmpty) {
        // first letter of the search term is capitalized
        String searchTermCapitalized =
      ...
      querySnapshot.docs.forEach((doc) {
        setState(() {
          searchResultsCreators.add(doc['name']);
        });
      });
    } catch (e) {
      print('Error searching creators: $e');
    }
  }
          ...
          child: Padding(
            ...
            child: Column(
              ...
              children: [
                ...
                if (searchResultsCreators.isNotEmpty)
                  ListView.builder(
                    shrinkWrap: true,
                    itemCount: searchResultsCreators.length,
                    itemBuilder: (context, index) {
                      String creatorName = searchResultsCreators[index];
                      return ListTile(
                        title: Text(creatorName),
                        onTap: () {
                          navigateToProfilePage(creatorName);
                        },
                      );
                    },
                  ),
